...
<?php
   ...
   // $owner_add = $_POST['o_add'];
    ...
    if(in_array($fileType1, $allowTypes)&& in_array($fileType2, $allowTypes)&& in_array($fileType3, $allowTypes)&& in_array($fileType4, $allowTypes)&& in_array($fileType5, $allowTypes) && in_array($fileType6, $allowTypes)){
     ...
     $sql="INSERT INTO hostel_owner (h_name, h_add, h_desc, h_services, h_seat, h_seat_price, o_name, o_email, o_contactNo, h_pass) VALUES ('$h_name', '$h_add', '$h_desc', '$h_services', '$h_seat', '$h_price', '$owner_name', '$owner_email', '$owner_mob', '$owner_password')";
   // $sql="INSERT INTO hostel_owner (h_name, h_add, h_desc, h_services, h_seat, h_seat_price) VALUES ('$h_name', '$h_add', '$h_desc', '$h_services', '$h_seat', '$h_price')";

    $insert = $conn->query($sql);

     $update = $conn->query("UPDATE hostel_owner SET h1_img = '$img1Content',h2_img = '$img2Content',h3_img = '$img3Content', h4_img = '$img4Content', h5_img = '$img5Content', h_qr = '$img6Content',h_request='Not Verified' WHERE o_email = '$owner_email'");

     

      if($insert&&$update){
         header("Location: h_owner_login.php", true, 301);  
         exit();
       }else{
         echo"error";
      } 
